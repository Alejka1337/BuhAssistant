...
export default function PostItem({
  ...
  useEffect(() => {
    // Если это не оптимистичное обновление, синхронизируем с сервером
    if (!isOptimisticUpdate.current) {
      setIsLiked(post.is_liked_by_user || false);
      setLikesCount(post.likes_count || 0);
    } else {
      // Сбрасываем флаг после получения данных с сервера
  ...
  const handleLike = () => {
    ...
    // Устанавливаем флаг оптимистичного обновления
  ...
  return (
    <View style={[styles.container, post.replies && post.replies.length > 0 && styles.containerWithReplies]}>
      ...
      {isReply && parentPost && (
        <TouchableOpacity style={styles.quote} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
  ...
  containerWithReplies: {
    marginBottom: Spacing.sm, // Больший отступ если есть ответы
  },
  header: {
    flexDirection: 'row',
  ...
  date: {
    ...Typography.caption,
    color: Colors.primary, // Зелёная дата
    fontWeight: '600',
    fontSize: 12,
  },
  edited: {
  ...
  quote: {
    ...
    padding: Spacing.xs,
    marginBottom: Spacing.xs,
    marginLeft: 12, // Среднее между 0 и 48
  },
  quoteLine: {
    width: 3,
  ...
  content: {
    ...
    lineHeight: 20,
    marginLeft: 12, // Среднее между 0 и 48
    marginBottom: Spacing.sm, // Увеличенный отступ перед actions
  },
  actions: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  replies: {
    ...
    marginLeft: -8, // Без смещения вправо
  },
  lastReply: {
    marginBottom: -Spacing.xs, // Уменьшаем отступ снизу для последнего ответа
  },
});
