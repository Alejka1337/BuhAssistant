"""
Краулер для парсинга новостей с сайта diia.gov.ua
Использует Playwright для обхода SPA и динамической загрузки контента
"""
...
# Константы
DIIA_GOV_UA_URL = "https://diia.gov.ua/news"
SOURCE_NAME = "diia.gov.ua"


class DiiaGovUaPlaywrightCrawler:
    """Краулер для diia.gov.ua с использованием Playwright"""
    ...
    async def initialize(self):
        ...
        logger.info("Инициализация Playwright браузера для Diia...")
        ...
        logger.info("Playwright браузер для Diia инициализирован")
    ...
    async def close(self):
        ...
        logger.info("Playwright браузер для Diia закрыт")
    ...
    async def parse_news_list(self) -> List[Dict]:
        ...
        logger.info(f"Переход на страницу: {DIIA_GOV_UA_URL}")
        
        try:
            # Переходим на страницу новостей
            response = await self.page.goto(DIIA_GOV_UA_URL, wait_until='networkidle')
            ...
            try:
                await self.page.wait_for_selector('div.posts_item', timeout=15000)
            except Exception as e:
                logger.warning(f"Timeout ожидания новостей: {e}")
                # Попробуем продолжить, возможно элементы уже загружены
            
            # Дополнительная задержка для загрузки всех элементов (SPA)
            await asyncio.sleep(2)
            
            # Извлекаем данные новостей
            news_items = await self.page.evaluate('''() => {
                const items = [];
                const newsBlocks = document.querySelectorAll('div.posts_item');
                
                newsBlocks.forEach(block => {
                    try {
                        // Ищем ссылку и заголовок
                        const linkEl = block.querySelector('a.posts_item-hover');
                        if (!linkEl) return;
                        
                        const href = linkEl.getAttribute('href');
                        const titleEl = linkEl.querySelector('span.posts_item-title');
                        const title = titleEl ? titleEl.textContent.trim() : null;
                        
                        // Ищем дату (может быть вне ссылки, в самом блоке)
                        const dateEl = block.querySelector('div.posts_item-date');
            ...
            for item in news_items:
                ...
                processed_items.append({
                    ...
                    'date': item.get('date', ''),
                    'source': SOURCE_NAME,
                    'raw_date': item.get('date', ''),  # Сохраняем оригинальную дату для парсинга
                })
    ...
    def parse_ukrainian_date(self, date_str: str) -> Optional[datetime]:
        """
        Парсинг украинской даты в различных форматах
        
        Args:
            date_str: Строка с датой на украинском (например: "21/11/2025")
...
async def crawl_diia_gov_ua() -> List[Dict]:
    """
    Основная функция для парсинга diia.gov.ua
    
    Returns:
        List[Dict]: Список новостей
    """
    async with DiiaGovUaPlaywrightCrawler() as crawler:
        ...
        logger.info(f"Успешно спарсено {len(news_items)} новостей с diia.gov.ua")
        return news_items
